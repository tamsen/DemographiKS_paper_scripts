...
class TestModelEffectsOfCLT(unittest.TestCase):
    ...
    def test_fractional_RC(self):
        ...
        for percent_RC in [0.1,0.2,0.3,0.5,0.75]:
            ...
            gain2 = [bin_size*wgd_lognorm2(i,num_genes,shape,loc,scale)
                       for i in bins_centers]

            param_string = str(bin_size) + " " + str(num_genes) + " " + str(shape) + \
                           " " + str(loc) + " " + str(scale)

            new_dist=[some_RC_loss[b]+ percent_RC*gain2[b]  for b in range(0,len(bins_centers))]
    ...
    def test_effects_of_CLT(self):
        ...
        blue_lightening=[0.75,0.65,0.6,0.4,0.3]
...
def plot_composite_tc_2(list_of_simulated_data, p0, two_Ne, num_genes,bins,
    ...
    scale = 2500  # should be about 2Ne (from Kingman std dev) but booted over for lognorm
    # scale from (1000*sqrt(2*pi)). see https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.lognorm.html
    RC_once_on_centers = [bin_size * wgd_lognorm2(i, num_genes, shape, loc, scale)
             for i in bins_centers]
    ...
    for i in range(0, len(list_of_simulated_data)):
        ...
        if i > 3:
            ...
            if "%" in data_label:
                q = num_RC
                p=1-q
                new_dist = [p*kingman_on_centers[b] + q* RC_once_on_centers[b]
                            for b in range(0, len(bins_centers))]
                max_value=max(new_dist)
                max_bin_index = new_dist.index(max_value)
                max_x_exp = bins_centers[max_bin_index+1]#symmetry issue, multiple max
                #max_x_exp = max(bins_centers[0],max_x_exp)
                #ax[0].plot(bins_centers, new_dist, color=color, alpha=0.95, label=str(num_RC) + '_new_dist',
                #     linestyle="-")
            else:
