...
class TestResampleTc(unittest.TestCase):
    ...
    def test_ResampleTc_With_Panmixia(self):

        output_folder = "/home/tamsen/Data/DemographiKS_output_from_mesx/KS_vs_Inb/trees_file_testing"
        trees_file="allotetraploid_bottleneck_trees_at_div_32.txt"
        png_number="32"
        type_of_inbreeding="random mating"
        self.make_Tc_comparison_plots(output_folder, png_number, trees_file, type_of_inbreeding)
        self.assertEqual(True, True)  # add assertion here

    def test_ResampleTc_With_Inbreeding(self):

        output_folder = "/home/tamsen/Data/DemographiKS_output_from_mesx/KS_vs_Inb/trees_file_testing"
        trees_file = "allotetraploid_bottleneck_trees_at_div_33.txt"
        png_number = "33"
        type_of_inbreeding = "strong inbreeding"
        self.make_Tc_comparison_plots(output_folder, png_number, trees_file, type_of_inbreeding)
        self.assertEqual(True, True)  # add assertion here

    def make_Tc_comparison_plots(self, output_folder,
                                 png_number, trees_file, type_of_inbreeding):
        ...
        individual_number = 0
        for pair in pairs:
            individual_number = individual_number + 1
            ...
            label_for_pair = "individual #" + str(individual_number) + \
                             ": genomes " + str(pair[0]) + " and " + str(pair[1])
        ...
        png_out = os.path.join(output_folder,type_of_inbreeding + "mrca_hist_overlay_"
                               + str(png_number) + "_Fig.S-Inb.png")
        plot_composite_mrca(data_list, bins, data_labels, type_of_inbreeding, png_out)


def plot_composite_mrca(data_list, bins, data_labels,type_of_inbreeding, png_out):

    fig = plt.figure(figsize=(5,5), dpi=350)
    label = "Comparison of coalescent distributions \nfor 4 sampled ancestral individuals\n" + \
            "under " + type_of_inbreeding
